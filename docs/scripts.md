...
| `scripts/replay.sh replay [OPTIONS] LOG...` | Re-run the detector over recorded training session logs (`.jsonl[.gz]`) and print reminders, false-alarm overlaps, detected labels, UNKNOWN fraction and live/replay agreement; `--trigger-ms/--clear-ms/--cooldown-ms/--window-ms` for tuning experiments. `scripts/replay.sh demo-log OUT [LOOPS]` writes the labelled synthetic session. See [data.md](data.md). |
| `python3 scripts/chime.py [--check]` | Regenerate the reminder chime `app/src/main/res/raw/chime.wav` deterministically (two soft notes, 22.05 kHz mono); `--check` (run by the pipeline) fails if the committed file differs. |
