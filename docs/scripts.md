...
| `scripts/lint.sh [TOOL...]` | Run the non-Gradle linters in pinned containers: `reuse` (REUSE 3.3 / SPDX), `ruff` (check + format), `yaml` (yamllint), `xml` (xmllint), `docker` (hadolint), `actions` (actionlint), `markdown` (markdownlint-cli2). Default: all; e.g. `scripts/lint.sh reuse ruff`. Builds the toolbox image from `docker/lint/Dockerfile`. The pipeline's *Lint Suite* stage runs it. |
| `python3 scripts/sbom.py build [OUT]` / `notices [--check]` | Turn the Gradle CycloneDX SBOM (`./gradlew :app:cyclonedxDirectBom`) into the release SBOM with the app and bundled models (`build/sbom/<name>-<version>.cdx.json`), and derive the About screen's component list `app/src/main/assets/third_party.json`; `--check` (pipeline stage *SBOM*) fails when that list differs. See [licensing.md](licensing.md). |
| `./gradlew detekt` | Kotlin static analysis over all modules with [`detekt.yml`](../detekt.yml) (every deviation from the defaults explained there). The pipeline's *Detekt* stage runs it. |
